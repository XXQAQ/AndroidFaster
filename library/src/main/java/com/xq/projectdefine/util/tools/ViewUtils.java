...
public class ViewUtils {
    ...
    public static void autoCountDown(final Context context, final TextView view, final int disEnableBackgroundRes, final int disEnableTextColorRes, final String fomat, final int time, final OnCountDownListener listener) {

        final Drawable enableBackground = view.getBackground();
        final ColorStateList enableTextColor = view.getTextColors();
        final String enableText = (String) view.getText();
            ...
            public void run() {
                try {
                    ...
                    ((Activity) context).runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            view.setBackgroundResource(disEnableBackgroundRes);
                            view.setTextColor(context.getResources().getColor(disEnableTextColorRes));
                    ...
                    for (int i = time; i > 0; i--) {
                        ...
                        ((Activity) context).runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                view.setText(String.format(fomat, i_copy));
                            }
                        });
                        Thread.sleep(1000);
                    }
                    ...
                    ((Activity) context).runOnUiThread(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            view.setBackground(enableBackground);
                            view.setTextColor(enableTextColor);
