...
addPaddingTopEqualStatusBarHeight    : 为 view 增加 PaddingTop 为状态栏高度
...
```
getPsuedoUniqueID  : 获取设备UUID
shutdown           : 关机
reboot             : 重启
reboot2Recovery    : 重启至Recovery
reboot2Bootloader  : 重启至Bootloader
isPhone            : 判断设备是否是手机
getDeviceId        : 获取设备码
getSerial          : 获取序列号
getIMEI            : 获取 IMEI 码
...
```
getMailIntent                    : 获取邮件意图
getWebIntent                     : 获取WebView意图
getContactsIntent                : 获取联系人意图
getSettingsIntent                : 获取系统设置意图
...
```
getBoolean, getFraction, getInteger, getIntArray, getDimension, getString, getStringArray, getDrawable, getBitmap : 根据resId获取资源文件
getIdByName, getStringIdByName, getColorIdByName, getDimenIdByName, getDrawableIdByName, getMipmapIdByName, getLayoutIdByName, getStyleIdByName, getAnimIdByName, getAnimatorIdByName, getMenuIdByName : 根据res名称获取资源文件
...
```
runOnChildThread        : 在默认线程池中执行子线程任务
runOnUiThread           : 在主线程中执行任务
runOnUiThreadDelayed    : 在主线程中执行延时任务
...
```
getUriForFile: file 转 uri
getFileForUri: uri 转 file
```

* ### 震动相关 -> [VibrateUtils.java]
```
vibrate: 震动
cancel : 取消
```
